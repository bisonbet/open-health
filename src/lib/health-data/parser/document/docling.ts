...
export class DoclingDocumentParser extends BaseDocumentParser {
  ...
  /**
   * OCR method. If `options.input` is an HTTP(S) URL, we download it first,
   * otherwise we assume it's a local file path and create a read stream.
   */
  async ocr(options: DocumentOCROptions): Promise<OCRParseResult> {
    ...
    const fileName = "defaultfile.pdf"; // Name to present to docling-serve

    if (inputPath.startsWith("http://") || inputPath.startsWith("https://")) {
      // --- REMOTE FILE: fetch & buffer ---
    ...
    if (!response.ok) {
      ...
      throw new Error(`Docling server error (OCR): ${response.status} - ${response.statusText}\n${errText}`);
    }
  ...
  /**
   * parse method. Same logic: if `options.input` is a URL, download first.
   */
